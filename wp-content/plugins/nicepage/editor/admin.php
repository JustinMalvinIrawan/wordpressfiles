...
class NpAdminActions {
    ...
    /**
     * Get posts by categories for blog control
     *
     * @param array $sources
     *
     * @return array $result
     */
    public static function getBlog($sources) {
        $result = array();
        ...
        foreach ($sources as $key => $source) {
            $source = count($sources) === $key + 1 ? 'Recent posts' : $source;
            $posts = self::getBlogPosts($source);
            if (count($posts) > 0) {
                if (preg_match('/^tags:/', $source)) {
                    array_push($result, array('tags' => str_replace('tags:', '', $source), 'id' => null, 'posts' => $posts));
                } else {
                    array_push($result, array('category' => $source, 'id' => get_cat_ID($source), 'posts' => $posts));
                }
            }
        }
        return $result;
    ...
    public static function getPostsCategories() {
        ...
        if (preg_match_all('/<\!--blog-->([\s\S]+?)<\!--\/blog-->/', $pagePublishHtml, $blogMatches, PREG_SET_ORDER)) {
     ...
     * @return array $result
     */
    public static function getPosts($source = false, $numberposts = 20) {
        $posts = array();
        $cat_id = '';
        $tags = '';
        if (preg_match('/^tags:/', $source)) {
    ...
    /**
     * Get posts for blog control
     *
     * @param string $category
     *
     * @return array
     */
    public static function getBlogPosts($category) {
        $posts = array();
        $category = $category === 'Recent posts' ? false : $category;
        // if recent posts - get last posts
        $items = self::getPosts($category);
    ...
    public static function getBlogPost($item) {
        ...
        $data = array(
            ...
            'post-readmore-text' => __('Read more', 'default'),
    ...
    public static function printBridgeAction() {
        ...
        $settings = array(
            'actions' => array(
                ...
                'getPosts'               => NpAction::getActionUrl('np_get_blog_posts'),
            ),
            'ajaxData' => array(
                'uid' => $uid,
        ...
        $data['info'] = array(
            ...
            'blogInfo' => self::getBlog(self::getPostsCategories()),
