...
function handleLinkCheckboxChange(event) {
    ...
    if (value === "section") {
        ...
        var phoneVal = urlField.find('input').val().indexOf('tel:') === 0 ? urlField.find('input').val() : '';
        ...
        var emailVal = urlField.find('input').val().indexOf('mailto:') === 0 ? urlField.find('input').val() : '';
        var emailParts = emailVal.replace('mailto:', '').split('?subject=');
        if (emailParts.length) {
            emailField.find('input').val(emailParts[0]);
            emailSubjectField.find('input').val(emailParts[1] || '');
        }
        urlCheckbox.addClass('hidden');
        urlField.addClass('hidden');
        pageList.addClass('hidden').css({"display": "none"});
...
function addAnchorsToLinkDialog(dialog, data) {
    if (!data || !data.l || !data.l.anchorLink || !data.anchorsList || !data.anchorsList.length || !data.l.fileLink) {
        return;
    }
    ...
    dialog.find('#wp-link-phone').on('change', setterUrl);
    dialog.find('#wp-link-email').on('change', setterUrl);
    dialog.find('#wp-link-email-subject').on('change', setterUrl);
    ...
    if (window.dataForDialog.isMenu) {
        jQuery('.file-upload-parent').addClass('hidden');
    }
...
function setterUrl(event) {
    var url = jQuery('#wp-link-url'),
        email = jQuery('#wp-link-email'),
        subject = jQuery('#wp-link-email-subject'),
        subjectVal = '';
    if (event.target.getAttribute('id') == "wp-link-phone") {
        url.val('tel:' + jQuery(this).val());
    } else {
    ...
    if (data.action === 'close') {
        ...
        wpLink.update = function () {
            ...
            if (linkType == 'email') {
                ...
                var valid = /^[\w-\.]+@[\w-]+\.[a-z]{2,4}$/i.test(emailValue);
                if (!valid) {
